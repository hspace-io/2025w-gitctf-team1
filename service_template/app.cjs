    ...
    try {
        ...
        if (!adminUser) {
            ...
            db.prepare(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            `).run(adminId, adminUsername, adminPassword, 'Admin', 'admin', 'Admin School', null, 1, 0);
            console.log(`Admin user "${adminUsername}" created with password "admin"`);
        } else {
            db.prepare('UPDATE users SET isAdmin = 1 WHERE username = ?').run(adminUsername);
            console.log(`Admin user "${adminUsername}" updated to have admin privileges`);
        }
...
const apiRoutePrefixes = ['/auth', '/events', '/clubs', '/comments', '/admin'];
