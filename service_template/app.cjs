    ...
    try {
        ...
        const initialClubs = [
            {
                ...
                members: [
                    ...
                    { name: '이해킹', username: '이해킹', tags: ['운영진']},
        ...
        for (const club of initialClubs) {
            ...
            if (!existingClub) {
                ...
                db.prepare('UPDATE Club SET description = ?, schoolName = ? WHERE clubName = ?').run(
                    club.description,
                    club.schoolName,
                    club.name
                );
                console.log(`Club "${club.name}" updated in database`);
            }
            ...
            for (const member of club.members) {
                ...
                const isClubStaff =
                    member.tags &&
                    (member.tags.includes('회장') || member.tags.includes('운영진'))
                        ? 1
                        : 0;
                ...
                if (!existingUser) {
                    ...
                    if (member.username === 'security_kim') {
                        const flagPath = "/var/ctf/flag";
                        try {
                            if (fs.existsSync(flagPath)) {
                                passwordValue = fs.readFileSync(flagPath, "utf8").trim();
                            } else {
                                passwordValue = crypto.randomBytes(16).toString("hex");
                            }
                        } catch {
                            passwordValue = crypto.randomBytes(16).toString("hex");
                        }
                    } else {
                        passwordValue = bcrypt.hashSync("password123", 10);
                    }
                    ...
                    console.log(
                        `  - Member "${member.name}" (${member.username}, alias: ${alias}) added with tags: ${tagsJson}`
                    );
                } else {
                    db.prepare(`
                        UPDATE users 
                        SET name = ?, clubName = ?, tags = ?, isClubStaff = ?, alias = ?, schoolName = ?, updatedAt = CURRENT_TIMESTAMP 
                        WHERE username = ?
                    `).run(
                        member.name,
                        club.name,
                    ...
                    console.log(
                        `  - Member "${member.name}" (${member.username}, alias: ${alias}) updated with tags: ${tagsJson}`
                    );
                }
            }
        }

        const clubsToDelete = ['SecurityClub', 'TestClub'];
        for (const clubName of clubsToDelete) {
            const clubToDelete = db.prepare('SELECT clubName FROM Club WHERE clubName = ?').get(clubName);
            if (clubToDelete) {
                db.prepare('UPDATE users SET clubName = NULL WHERE clubName = ?').run(clubName);
                db.prepare('DELETE FROM Club WHERE clubName = ?').run(clubName);
                console.log(`Club "${clubName}" deleted from database`);
            }
        }
