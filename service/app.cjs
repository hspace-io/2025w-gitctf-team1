...
if (!DEFAULT_SEED_PASSWORD) {
    console.error("FATAL ERROR: SEED_PASSWORD is missing in environment variables.");
    process.exit(1);
}

const hashedDefaultPassword = bcrypt.hashSync(DEFAULT_SEED_PASSWORD, 10);
    ...
    try {
        ...
        for (const club of initialClubs) {
            ...
            if (!existingClub) {
                ...
                const defaultPassword = hashedDefaultPassword;
